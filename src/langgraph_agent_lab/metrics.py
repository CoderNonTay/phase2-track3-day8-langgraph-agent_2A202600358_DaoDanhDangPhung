...
def metric_from_state(
    ...
    # interrupt_count = approval events that actually used interrupt() (real HITL),
    # falls back to count of approval events otherwise so metrics remain meaningful.
    interrupt_events = [
        event
        for event in events
        if event.get("node") == "approval"
        and (event.get("metadata") or {}).get("used_interrupt")
    ]
    if interrupt_events:
